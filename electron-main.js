...
function attachOutsideClickCloser() {
  try {
    ...
    if (omniboxCloseHandler) return; // already attached
    omniboxCloseHandler = (_event, input) => {
      if (!omniboxVisible) return;
      // Close on any mouse click in the content area
    ...
    browserView.webContents.on('before-input-event', omniboxCloseHandler);
  } catch {}
}

function detachOutsideClickCloser() {
  try {
    if (omniboxCloseHandler && browserView && browserView.webContents) {
      browserView.webContents.removeListener('before-input-event', omniboxCloseHandler);
    }
  } catch {}
...
function showMacroExecutionOverlay(macroName, progress, description, screenshot) {
  ...
  try {
    ...
    console.error('[Macro Overlay] Overlay injection error:', err);
  }
}
