...
class MacroExecutor extends EventEmitter {
  ...
  async executeClick(step) {
    ...
    // Execute click in page context (using JSON.stringify to safely escape selector)
  ...
  async executeInput(step, result) {
    ...
    // Execute input in page context (using JSON.stringify to safely escape both selector and value)
  ...
  async executeWait(step) {
    ...
    if (!step.condition || step.condition === 'time') {
      ...
      await this.waitForElement(step.selector, step.timeout || 10000);
  ...
  async waitForElement(selector, timeout) {
    ...
    while (Date.now() - startTime < timeout) {
      ...
      try {
        ...
        if (result) {
          console.log('[MacroExecutor] Element is visible:', selector);
          return;
        }
      } catch (err) {
        // Continue waiting
